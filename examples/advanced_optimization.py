...
def main():
    ...
    print(
        f"  - Diámetro: {result.optimal_geometry.diameter:.4f} m = {result.optimal_geometry.diameter*1000:.2f} mm"
    )
    print(
        f"  - Distancia focal: {result.optimal_geometry.focal_length:.4f} m = {result.optimal_geometry.focal_length*1000:.2f} mm"
    )
    print(
        f"  - Profundidad: {result.optimal_geometry.depth:.4f} m = {result.optimal_geometry.depth*1000:.2f} mm"
    )
    ...
    print(
        f"  - Cumple restricción: {'✓' if weight_kg <= constraints.max_weight else '✗'}"
    )
    print()
    ...
    if len(result.convergence_history) > 0:
        ...
        print(
            f"  - Mejora total: {improvement:.3f} dB ({improvement/result.convergence_history[0]*100:.1f}%)"
        )

        # Detectar estancamiento
        last_20_pct = max(1, len(result.convergence_history) // 5)
        recent_improvement = (
            result.convergence_history[-1] - result.convergence_history[-last_20_pct]
        )
        print(
            f"  - Mejora últimas {last_20_pct} generaciones: {recent_improvement:.4f} dB"
        )
