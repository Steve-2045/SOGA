...
def main():
    ...
    print(f"  - Diámetro: {params['min_diameter_m']:.2f} - {params['max_diameter_m']:.2f} m")
    print(f"  - Peso máximo: {params['max_payload_g']:.0f} g")
    print(f"  - Relación f/D: {params['min_f_d_ratio']:.2f} - {params['max_f_d_ratio']:.2f}")
    ...
    print(f"  - Diámetro: {result['optimal_diameter_mm']:.2f} mm ({result['optimal_diameter_mm']/10:.1f} cm)")
    print(f"  - Distancia focal: {result['optimal_focal_length_mm']:.2f} mm ({result['optimal_focal_length_mm']/10:.1f} cm)")
    print(f"  - Profundidad: {result['optimal_depth_mm']:.2f} mm ({result['optimal_depth_mm']/10:.1f} cm)")
    ...
    if result['convergence']:
        ...
        print(f"  - Mejora: {result['convergence'][-1] - result['convergence'][0]:.2f} dB")
    print()
    ...
    print(f"  - Cumple restricción de peso: {'✓ SÍ' if estimated_weight_g <= params['max_payload_g'] else '✗ NO'}")
    print()
